...
def run_pipeline(save: bool = True) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Load synthetic data -> engineer features -> temporal split -> (optionally) save."""
    ...
    # NOTE: once Harespod data is available, this is the point where we'd
    # call harespod_loader.load_all_subjects(), apply the same LLS-derived
    # labeling rules (currently only implemented for synthetic generation
    # since it controls its own labels; a real-data labeling pass would live
    # in a `label_harespod.py` sibling module), and pd.concat with `raw`
    # before feature engineering -- everything below this line is already
    # source-agnostic.
