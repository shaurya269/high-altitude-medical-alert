...
**Primary source:** Harespod dataset (open, 15 subjects, hypobaric chamber, SpO₂ + HR +
respiration at 100Hz). Download from Figshare (DOI: 10.6084/m9.figshare.c.6623344.v1)
and companion code at https://github.com/oca-john/Harespod.

**Gaps in Harespod, and how each is filled:**
| Gap | Fill |
|---|---|
| No body temperature | Synthesized from literature-based distributions conditioned on inferred severity |
| No continuous pressure/altitude | Interpolated from Harespod's altitude change markers |
| No HAPE/HACE cases (ethically can't induce in subjects) | Fully synthetic trajectories built from clinical literature ranges |
| No severity labels | LLS-derived rule mapping applied post-hoc |

**Pipeline (build in this order, Days 1–3):**
1. Load + clean + downsample Harespod to 1Hz (matches realistic Arduino sample rate)
2. Interpolate continuous altitude from markers
3. Generate synthetic temperature column
4. Generate fully synthetic HAPE/HACE trajectories (gradual onset, not step-function)
5. Apply LLS-derived severity labeling rules
...
| LLM provider | Groq (Llama 3.3 70B) | Confirmed, API key already obtained |
| Dataset | Harespod + synthetic augmentation | Confirmed |
| Alert channel | Telegram bot | Confirmed |
